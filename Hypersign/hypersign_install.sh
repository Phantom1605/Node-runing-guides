...
# set vars
if [ ! $HID_MONIKER ]; then
	read -p "Enter node name: " HID_MONIKER
	echo 'export HID_MONIKER='$HID_MONIKER >> $HOME/.bash_profile
fi
HID_PORT=35
if [ ! $HID_WALLET ]; then
	read -p "Enter wallet name: " HID_WALLET
	echo 'export HID_WALLET='$HID_WALLET >> $HOME/.bash_profile
fi	
echo "export HID_CHAIN=jagrat" >> $HOME/.bash_profile
echo "export HID_PORT=${HID_PORT}" >> $HOME/.bash_profile
source $HOME/.bash_profile

echo '================================================='
echo -e "Your node name: \e[1m\e[32m$HID_MONIKER\e[0m"
echo -e "Your wallet name: \e[1m\e[32m$HID_WALLET\e[0m"
echo -e "Your chain name: \e[1m\e[32m$HID_CHAIN\e[0m"
echo -e "Your port: \e[1m\e[32m$HID_PORT\e[0m"
echo '================================================='
sleep 2
...
# packages
sudo apt install curl build-essential git wget jq make gcc tmux mc htop chrony -y
...
# config
hid-noded config chain-id $HID_CHAIN
hid-noded config node tcp://localhost:${HID_PORT}657
...
echo -e "To check sync status: \e[1m\e[32mcurl -s localhost:${HID_PORT}657/status | jq .result.sync_info\e[0m"
